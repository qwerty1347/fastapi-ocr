...
FastAPI 기반의 OCR 서비스로, EasyOCR, PaddleOCR, Clova OCR 등 다양한 OCR 엔진을 지원합니다. 이미지에서 텍스트를 추출하는 API를 제공하며, 모듈화된 구조로 쉽게 확장이 가능합니다.
...
- 다양한 OCR 엔진 지원 (EasyOCR, PaddleOCR, Clova OCR)
...
│   ├── domain/                  # 도메인 로직
│   │   └── ocr/                 # OCR 관련 코드
│   │       ├── engines/         # OCR 엔진 구현체 (EasyOCR, PaddleOCR, Clova OCR)
│   │       │   ├── base_engine.py
│   │       │   ├── easyocr.py
│   │       │   ├── paddleocr.py
│   │       │   └── clovaocr.py
